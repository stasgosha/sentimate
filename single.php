...
<script>
    (function ($) {
        ...
        function textToId(str){
            return str.toLowerCase().replaceAll('-', '').replaceAll(' ', '-').replaceAll('?', '').replaceAll(':','').replace(/([.*+?^=!:${}()|\[\]\/\\])/g, "").replace(new RegExp('[0-9]'), 'a');
        }
        ...
        $('.entry-content-table h2').each(function(i, el) {
            ...
            $(el).find('~ h2, ~h3').each(function(i, h){
                ...
                const textH3 = $(h).text();
                const handleH3 = textToId( textH2 );

                subnavItemsHTML += `<li><a href="#${handleH3}">${textH3}</a></li>`;
            });


            const subnavHTML = !!subnavItemsHTML ? `<ul>${ subnavItemsHTML }</ul>` : '';
        ...
        // let tableOfContentsList = $('.table-of-contents-list');
        //
        // $('.entry-content-table h2, .entry-content-table h3').each(function() {
        //     let singleContent = document.createElement('li');
        //     let handle = $(this).text().toLowerCase().replaceAll('-', '').replaceAll(' ', '-').replaceAll('?', '').replaceAll(':','').replace(/([.*+?^=!:${}()|\[\]\/\\])/g, "").replace(new RegExp('[0-9]'), 'a');
        //     let text = $(this).text();
        //     let link = `<a href="#${handle}">${text}</a>`;
        //     $(singleContent).append(link);
        //     $(tableOfContentsList).append(singleContent);
        //     $(this).attr('id', handle);
        // })
        $(tableOfContents).append(tableOfContentsList);
        ...
        $('body').on('click', '.table-of-contents a', function(e) {
            ...
            var target_top = $($(this).attr('href')).offset().top - $('.header').outerHeight() -  $('.table-of-contents').outerHeight();
