...
(function($) {
    function drawOverview(parentSelectorId, productGuid, urlParams, currentProduct) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                console.log(data);
                if ( !urlParams.has('productSlug')) {
                    urlParams.append('productSlug', data["name"].replaceAll(' ', '-'));
                ...
                $(parentSelectorId + ' .prod_img img').attr('src', data["image"]);
                $(parentSelectorId + ' .prod_title').text(data["name"]);

                let category  = data["category"].split('|');
                    category  = category[category.length - 1];
                let tmpLogo   = 'https://content.revuze.it/brands/' + data.brand_name.toLowerCase().replaceAll(' ', '-') + '.jpg';
                let brandLogo = data.brand_logo === null ? tmpLogo : data.brand_logo;
                console.log(brandLogo);

                if (parentSelectorId.includes('productOverview1')) {
                    $('img[data-id="brandLogo1"]').attr('src', brandLogo );
                }
                if (parentSelectorId.includes('productOverview2')) {
                    $('img[data-id="brandLogo2"]').attr('src', brandLogo );
                }

                $(parentSelectorId + ' img[data-id="brandLogo"]').attr('src', brandLogo );
                $(parentSelectorId + ' [data-id=brand]').text(data["brand_name"]);
                $(parentSelectorId + ' [data-id=industry]').text(data["industry_name"]);
                $(parentSelectorId + ' [data-id=category]').text(category);


                currentProduct["tooltip"]["name"] = data["name"];
                currentProduct["uuid"] = productGuid;
            },
            complete: function() {
                let biggestHeight = 0;
                ...
                if (parentSelectorId.includes('productOverview1')) {
                    ...
                    $('div[data-id="productTitle1"]').text(currentProduct.tooltip.name);
                }
                if (parentSelectorId.includes('productOverview2')) {
                    $('div[data-id="productTitle2"]').text(currentProduct.tooltip.name);
                }
            },
            error: function () {
                console.log("Error");
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                var chart = am4core.create((document.querySelector(parentSelectorId + ' #satisfaction')), am4charts.XYChart);
                var tempData = [];
                gengraph(chart, 'date');
    ...
    function drawMegatopicSentiment(parentSelectorId, productGuid, urlParams) {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'get_topic_monthly_sentiment',
                product_id: productGuid,
            },
            success: function (response) {
                let json = JSON.parse(response);
    ...
    $(document).ready(function() {
        ...
        if (urlParams.has('productGuid')) {
            if ($('#singleProduct').length > 0) {
                currentProduct = drawOverview('#singleProduct #overview', urlParams.get('productGuid'), urlParams, currentProduct);
                drawCompetitors('#singleProduct #competitive_landscape', urlParams.get('productGuid'), urlParams, currentProduct);
                drawMegatopicSentiment('#singleProduct #product_satisfaction', urlParams.get('productGuid'), urlParams);
            }
        }
